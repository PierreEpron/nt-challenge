...
Pour ce faire :
- J'ai essayé différentes approches pour générer des données d'entrainements pertinentes.
- J'ai essayé différentes pipelines. À ce titre, j'ai essayé de comparer l'utilisation d'un ResponseSelector avec d'un modèle plus classique basée sur la détection d'entité.
...
J'utiliserais par la suite contact_doctor et contact_service pour designer les deux intentions.
...
- La capacité d'entraîner des models basées sur du yml accélère grandement les choses.
- Si je suis pris pour ce poste, j'aurai eu l'occasion d'avoir une première expérience avec Rasa.

Points négatifs :
- J'ai eu un peu de mal à déployer rasa x sur mon serveur. (notamment le server d'action)
- C'est une usine à gaz. J'aurais pu utiliser une solution plus simple :
    ...
    - Les autres frameworks spécialisé en NLU (comme Snpis, Luis) sont très similaire à rasa ou proposent des solutions payantes (Saga).
...
J'ai utilisé les composants spacy dans différentes pipeline.
J'ai aussi utilisé sa fonction de similarité (cosinus distance basé sur les vecteurs du word embbeding).

J'ai déjà eu l'occasion d'utilisée spacy durant ma periode d'alternance l'année dernière.
...
Leurs apports m'a été très utiles pour avoir une idée plus générale du type de phrase possible pour exprimer les deux intentions. Ils m'ont aussi permis de constituer un petit jeu de test que j'ai utilisé tout au long du développement, au fur et mesure que celui-ci s'enrichissait.

Le jeu de test pose quand même quelque soucis :
- Il y a un peu moins de 200 phrases utilisables ce qui, pour moi, n'est pas suffisant pour évaluer correctement un model.
- Les utilisateurs ont beaucoup plus cherché a piégé le model qu'à le tester. C'est intéressant pour comprendre ses défauts, mais ça biaise l'évaluation. Un jeu de donnée réelle contiendrait, à mon sens, beaucoup plus de phrases classique.
- Il est déséquilibré. J'ai eu beau donnée a chacun des docteurs et services différents, certaines valeurs sont surreprésenter et inexistante.


### Première tentative de generation
...
J'ai fait quelques tests avec ce dataset et j'ai vite compris plusieurs problèmes en le confrontant à des utilisateurs externes (Je ne détaillerais pas ici la pipeline utilisée, j'en parle plus loin.) :
- Il faut spécifier une intention out_of_scope sinon le model cherche toujours a catégoriser la phrase comme contact_doctor ou contact_service. C'est assez logique et au final, c'est plus un oubli de ma part qu'un réel problème.
- De manière générale, le model avait du mal si l'objet du contacte est avant la demande de contacte. C'était tout à fait normal, aucune phrase dans le jeu d'entraînement ne présentait ce genre de configuration.
- Ex : "J'ai rendez vous avec le docteur X. Pouvez-vous me le passer ?"
- Lorsque plusieurs objets étaient présents dans une même phrase. Le model gardait le dernier trouver comme target. La raison : je n'utilisais pas de rôle pour differencier les objets trouvés.
...
Faut-il différencier la prise de rdv d'une demande de contact classique ? La question se pose aussi dans d'autres cas. Faut-il considérer que ces phrases sont des demandes de service / médecin ? (phrases prisent dans le jeu de tests utilisateurs.)
...
Dans le cadre où le bot aurait comme objectif d'être mis en production dans un hôpital ou une clinique, la question serait réglé avec l'aide du client. Dans mon cas actuel, j'ai décidé de choisir moi-même, au cas par cas. L'objectif étant de voir si le bot s'en sort bien pour détecter des demandes de contact plus implicite.

### Deuxième tentative de generation

Après la première tentative, j'ai décidé d'utiliser le module chatette.

J'ai gardé l'idée des verbes de la première génération. J'ai généré via python un fichier chatette contenant les verbes et les règles de générations. C'était un défi intéressant, mais une grosse perte de temps avec du recul, je pense que l'impact de cette grande variété de verbe sur les performances du model est minime voir négative.

J'ai ajouté les possibilités :
- Qu'une phrase est du bruit au début et la fin (bonjour, s'il vous plaît, c'est urgent)
...
- Le jeu "0" :
    ...
    - Sois au total 2100 phrases.
- Le jeu "1" :
    - Contiens 50 phrases par sous-intention.
    ...
    - Sois au total 3600 phrases.

### Conclusion sur les données

Quand on se renseigne sur la NLU et plus spécifiquement sur la conception de chatbot. On lit partout qu'il faut éviter les données générées et qu'il faut préférer des données réelles. À ce titre, j'ai sans doute passé trop de temps à générer des données et pas assez à en produire moi-même. Si je devais recommencer, je ferais une génération plus simple et plus contrôler.

La partie la plus compliquée de la génération, c'est l'intention out_of_scope. Il est compliqué d'appréhender et de fournir au model l'ensemble des cas qu'il doit considérer comme n'étant pas exactement une demande de contact. La plupart des erreurs du model se font avec cette intention.
...
- Répondre en fonction l'intention et la valeur du slot
...
Les avantages :
- Le modèle est plus facilement généralisable. On peut s'attendre qu'il fonctionne bien en ajoutant des services / docteurs dans sa base de connaissance.
- Les données d'entraînement sont plus faciles à manipulés. (seulement 3 intentions)

Les inconvénients :
- La target peut-être complétement hors sujet.
...
L'autre approche est basée sur le composant ResponseSelector. Je me suis directement inspiré de la manière dont sont traité les FAQ par les chatbots. Pour faire simple, on définit une intention principale a lequel on ajoute une liste de sous-intention. Dans notre cas, la liste des docteurs comme sous-intentions de contact_doctor. Pareil pour les services.

Les avantages :
- Pas besoins de s'embêter avec de la similarité ou autre pour faire correspondre la target a une valeur exacte.
- Il est plus facile de catégoriser des phrases que d'annoter des entités. L'annotation amène toujours à des ambiguïtés et se doit d'être guidé par une guideline stricte pour avoir un model efficace.
...
- Contrairement à la similarité, on ne peut pas proposer les valeurs proches de la target si le modèle ne trouve pas exactement la target. Encore que, on pourrait ajouter un système de fallback qui proposerait les sous-entités avec les meilleurs scores si le model n'est pas sur du résultat.

https://rasa.com/docs/rasa/components#responseselector

### Spacy

Rasa propose d'intégrer facilement des composants de spacy. Celà peut-être utile pour utiliser un modèle pré entraîné ou un encoder plus élaborer.

Pour ma part, j'ai fait des tests avec :
- https://spacy.io/models/fr#fr_core_news_md
- https://spacy.io/models/fr#fr_dep_news_trf

Une des options possibles est d'utiliser le ner des pipelines spacy pour extraire les entités PER (personne) et les utilisées comme doctor. Je n'ai pas utilisé cette méthode par manque de temps et parce que je n'ai aucune idée de comment se comporterais le model en utilisant le NER d'une pipeline spacy et en définissant le rôle target.
...
- Knowledge Base Actions, j'ai découvert ce composant à la fin, quand j'ai développé les actions de similarités. Je pense que ça rentre de le cadre d'utilisation de ce bot et qu'il aurait été intéressant de mettre en place cette fonctionnalité.

Je n'ai pas eu le temps de tester l'approche une intention et/ou une entité. Elle consisterait à réunir docteur et service en une seul entité objet et/ou contact_service et contact_docteur en une seule intention contact. Je n'ai aucune idée de la faisabilité / viabilité de cette approche, mais il aurait été intéressant de s'y pencher./ viabilité de cette approche, mais il aurait été intéressant de s'y pencher.
...
La partie génération de phrase se trouve dans le notebook : sentences_generation.ipynb

Le code est fouilli et absolument pas commenter. Comme j'ai déjà expliqué plus haut, je ne suis pas sûr qu'elle vaille vraiment le coup d'être retravaillé. Il vaudrait mieux opter pour des templates chatette plus simple. À la rigueur, seule la partie qui génère le template main pour qu'il soit adapter au ResponseSelector est intéressant.
...
À la base, je voulais sortir des graphiques en output pour mieux visualiser les résultats, mais les boxplots et barchart sont peu lisible à cause des petites valeurs. Faire un histogramme pour chaque méthode aurait générer trop de fichiers.

Les résultats détaillés pour le jeu de test utilisateur sont trouvable a cet endroit : results/similarity_report.json
...
J'ai ajouté l'option --fill-role parce que je voulais tester si le model se comportait différemment en précisant un rôle quelconque aux entités qui ne sont pas des target.

#### Autres 

Le fichier src/libs/helpers.py qui contient quelques fonctions generique que j'utilise dans d'autres partie du code (docstring complète).

Le fichier train/train.sh, un script bash qui lance le train le model puis le test sur son jeu de test generer par chatette et sur le jeu de test utilisateur.
...
Cependant suite à des problèmes de déploiement avec le serveur actions. Les pipelines de type entity ne fonctionne pleinement qu'en local.
...
Les metrics par rasa pour evaluer la perfomance d'un model sont les suivantes.
...
Ce sont les métrics classique utilisaté dans une classification. Il existe des métrics plus complète pour évaluer un NER mais je ne les ai pas utilisée.

Chaque composant est évaluer indépendamment. Celà donne :
...
Les résulats sur le jeu de test generer par chatette sont tous très proche de 1 et présente peu d'intérêt à mon sens. D'une part parce que la géneration de phrase présente de gros défault, d'autre part parce que ce qui nous intéresse, c'est les performances du model sur des données proches du réelle.

Les résultats sont difficilement interpretable pour plusieurs raisons 
    - Le jeu de test contient seulement X phrase (X contact_doctor, X contact_service, X out_of_scope). C'est encore pire pour ce qui concerne le ResponseSelector, certain docteur ne sont même pas représenté.
    - Les utilisateurs test ont principalement cherché a pieger le model ce qui n'aide pas à l'évaluer correctement
    - L'annotation d'entité et la classification des intentions par une seule personne (moi) est souvent biaiser ou même incorrecte

On peut quand même constater ceci :


Les résultats complets pour chaque models sont trouvable dans les dossiers : train/*/*/results et train/*/*/user_results 
...
Une des causes est sûrement ma mauvaise organisation. J'avais comme suivi seulement des notes papiers et j'ai très peu planifié ce que je devais faire. J'avais des idées très larges, mais pas de route précise pour les réaliser.
...
La qualité du code a aussi souffert. Certaine partie mériterait d'être refactoriser d'autres d'être simplement commenté. J'aurais voulu faire des tests unitaires pour tout ce qui est similarités.
...
De manière générale, j'ai voulu testé trop choses en trop peu de temps. J'aurais dû me concentrer sur certains aspects et les réaliser plus proprement.
...
Je conclurais en vous remerciant d'avoir lu jusqu'au bout et en précisant qu'il est possible que je finisse certaine partie après vendredi 06/05 23h59 mais que tous les ajouts seront mis sur une autres branch que la branch main pour ne pas tricher.